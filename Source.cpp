...
void print_char_as_binary(char input) {
	int i;
	for (i = 0; i < 8; i++) {
		char shift_byte = 0x01 << (7 - i);//0000 0001 -> 1000 0000
		if (shift_byte & input) {
			printf("1");
		}
		else {
...
void generate_sub_keys(unsigned char* main_key, key_set* key_sets) {//генерация ключей
	int i, j;
	int shift_size;
	...
	for (i = 0; i < 56; i++) {
		shift_size = initial_key_permutaion[i];//перестановка
	...
	for (i = 1; i < 17; i++) { //сдвиги
		...
		for (j = 0; j < 48; j++) {
			shift_size = sub_key_permutation[j]; //формирование вектора CD
			if (shift_size <= 28) {
				shift_byte = 0x80 >> ((shift_size - 1) % 8);
...
void process_message(unsigned char* message_piece, unsigned char* processed_piece, key_set* key_sets, int mode) {
	...
	for (i = 0; i < 64; i++) { //начальная перестановка
	...
	for (k = 1; k <= 16; k++) { //ЦИКЛ 
		...
		for (i = 0; i < 48; i++) { //функция расширения Е
		...
		for (i = 0; i < 32; i++) { //перестановка в функции f
	...
	for (i = 0; i < 64; i++) {//конечная перестановка
